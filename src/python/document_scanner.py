...
class DocumentScanner:
    ...
    Implements edge detection, Hough Transform, and homography-based
    rectification.
    """
    ...
    def __init__(self, image_path: str):
        ...
        self.gray_image = cv2.cvtColor(
            self.input_image, cv2.COLOR_BGR2GRAY
        )
    ...
    def detect_edges(self) -> 'DocumentScanner':
        ...
        smoothed = cv2.GaussianBlur(
            self.gray_image, (kernel_size, kernel_size), self.sigma
        )
        ...
        # Threshold
        self.edge_image = (
            (grad_mag > self.edge_threshold).astype(np.uint8) * 255
        )
    ...
    def find_corners(
        self, manual_corners: Optional[np.ndarray] = None
    ) -> 'DocumentScanner':
        """
        Find corner points (manual or automatic).

        Args:
            manual_corners: Optional array of shape (4, 2) with corner
                          coordinates
        ...
        if manual_corners is not None:
            ...
            raise NotImplementedError(
                "Automatic corner detection not yet implemented"
            )

        return self

    def rectify_image(
        self, output_width: int = None, output_height: int = None
    ) -> 'DocumentScanner':
        """
    ...
    def visualize(self, step: str, save_path: Optional[str] = None):
        ...
        Args:
            step: Which step to visualize ('edges', 'hough', 'lines',
                  'corners', 'rectified')
        ...
        if step == 'edges':
            ...
            axes[0].imshow(cv2.cvtColor(
                self.input_image, cv2.COLOR_BGR2RGB
            ))
            ...
            # Draw corners
            ax.plot(
                self.corner_points[:, 0], self.corner_points[:, 1],
            ...
            for i in range(4):
                next_i = (i + 1) % 4
                ax.plot(
                    [self.corner_points[i, 0],
                     self.corner_points[next_i, 0]],
                    [self.corner_points[i, 1],
                     self.corner_points[next_i, 1]],
                    'r-', linewidth=2
                )
            ...
            axes[1].imshow(cv2.cvtColor(
                self.rectified_image, cv2.COLOR_BGR2RGB
            ))
    ...
    def save_rectified(self, output_path: str):
        ...
        if self.rectified_image is None:
            raise ValueError(
                "No rectified image to save. Run rectify_image() first."
            )
...
if __name__ == "__main__":
    ...
    scanner.visualize(
        'rectified', '../../outputs/rectified/rectified_python.png'
    )

    # Save final output
    scanner.save_rectified(
        '../../outputs/rectified/final_output_python.png'
    )
